...
def render_analysis():
    ...
    with tab_classes:
        ...
        with col_search:
            cls_search = st.text_input("🔍 클래스명 검색", placeholder="예) User", key="cls_search")
        with col_filter:
    ...
    with tab_methods:
        ...
        with col_search2:
            mtd_search = st.text_input("🔍 메소드명 검색", placeholder="예) find", key="mtd_search")
        with col_cls:
    ...
    with tab_calls:
        call_search = st.text_input("🔍 클래스명 검색", placeholder="예) Service", key="call_search")
